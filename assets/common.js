/* LUMREA — ortak çekirdek: yapılandırma, yardımcılar, veri modeli, yasal içerik */
"use strict";

const CONFIG = window.LUMREA_CONFIG || {};
...
const DEFAULT_CATEGORIES = ["Kombin", "Üst Giyim", "Alt Giyim", "Dış Giyim", "Elbise", "İç Giyim", "Aksesuar", "Ayakkabı"];
...
const defaultSettings = {
  ...
  supportEmail: "destek@lumrea.com", supportPhone: "0850 000 00 00",
  bankName: "", iban: "", accountHolder: "",
  companyName: "LUMREA Giyim", companyAddress: "",
  categories: DEFAULT_CATEGORIES,
  seoTitle: "LUMREA — Sade Giyin, Asil Kal",
...
function pageContent(key) {
  ...
  body = body.replaceAll("{SIRKET}", esc(s.companyName || "LUMREA Giyim"))
    .replaceAll("{ADRES}", esc(s.companyAddress || "—"))
    .replaceAll("{EPOSTA}", esc(s.supportEmail || "—"))
    .replaceAll("{TELEFON}", esc(s.supportPhone || "—"));
  return { title: custom?.title || base.title || "LUMREA", body };
}

/* ---------- durum ve kalıcılık ---------- */
function readLocal(key, fallback) { try { const v = localStorage.getItem(`lumrea_${key}`); return v ? JSON.parse(v) : structuredClone(fallback); } catch { return structuredClone(fallback); } }
function writeLocal(key, value) { localStorage.setItem(`lumrea_${key}`, JSON.stringify(value)); }
...
const state = {
  ...
  token: sessionStorage.getItem("lumrea_admin_token") || "",
...
async function apiCloud(path, options = {}) {
  ...
  try {
    ...
    if (seg[1] === "orders" && seg[2] && method === "DELETE") { return (await cloud.deleteOrder(decodeURIComponent(seg[2]))) || {}; }
