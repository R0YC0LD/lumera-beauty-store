...
(() => {
  ...
  function handleSessionExpiry(err) {
    if (err.status === 401) {
      state.token = ""; sessionStorage.removeItem("lumrea_admin_token");
  ...
  function productFormHtml(id) {
    const isNew = id === "new";
    const p = isNew
      ? { id: uid("g"), name: "", brand: "LUMREA", sku: "", category: getCategories()[0], gender: "unisex", price: "", oldPrice: "", badge: "", tags: [], tone: "#e8e2d6", description: "", fabric: "", care: "", imageUrl: "", criticalStock: state.settings.criticalStockDefault || 5, active: true, sizes: DEFAULT_SIZES.map(n => ({ name: n, stock: 0 })) }
    ...
    return `<div class="admin-panel"><h3>${isNew ? "Yeni ürün ekle" : `Düzenle: ${esc(p.name)}`}</h3>
      <form id="productForm" class="form-grid" data-product-id="${esc(p.id)}" data-is-new="${isNew}">
        ...
        <label class="field">Ürün kodu (SKU) *<input name="sku" required value="${esc(p.sku)}" placeholder="LMR-TS-001"></label>
  ...
  function seoHtml() {
    ...
    return `<div class="admin-panel"><h3>SEO ve sosyal medya</h3>
      <form id="seoForm" class="form-grid">
        ...
        <label class="field">Instagram bağlantısı<input name="instagram" value="${esc(s.instagram || "")}" placeholder="https://instagram.com/lumrea"></label>
        <label class="field">TikTok bağlantısı<input name="tiktok" value="${esc(s.tiktok || "")}" placeholder="https://tiktok.com/@lumrea"></label>
        <label class="field">X (Twitter) bağlantısı<input name="twitter" value="${esc(s.twitter || "")}" placeholder="https://x.com/lumrea"></label>
  ...
  function exportBackup() {
    const backup = {
      exportedAt: new Date().toISOString(), store: "LUMREA",
    ...
    download(`lumrea-yedek-${new Date().toISOString().slice(0, 10)}.json`, JSON.stringify(backup, null, 2));
    toast("Yedek indirildi");
  }
  async function importBackup(file) {
    let data;
    try { data = JSON.parse(await file.text()); } catch { toast("Dosya okunamadı — geçerli bir yedek dosyası seçin", false); return; }
    if (!Array.isArray(data.products)) { toast("Bu dosya bir LUMREA yedeği değil", false); return; }
  ...
  document.addEventListener("click", e => {
    ...
    let el;
    if (closest("#adminLogout")) { state.token = ""; sessionStorage.removeItem("lumrea_admin_token"); showLogin(); return; }
    ...
    if (closest("[data-export-subscribers]")) {
      ...
      download("lumrea-aboneler.csv", "email;kayit_tarihi\n" + rows, "text/csv");
      return;
    }
    /* kategoriler */
