...
function Brand({ compact = false }: { compact?: boolean }) {
  return (
    <span className={`brand ${compact ? "brand-compact" : ""}`} aria-label="Lumrea">
      <span className="brand-word">lumrea</span><i className="brand-dot" /><i className="brand-spark" />
  ...
  return <div className="storefront">
    ...
    <header className="site-header">
      <div className="header-main shell">
        ...
        <button className="logo-link" onClick={handleLogoClick} aria-label="Lumrea ana sayfa"><Brand /></button>
    ...
    <main id="top">
      <section className="hero">
        <img src="/lumera-hero.png" alt="Lumrea'nın seçilmiş bakım ve makyaj ürünleri" />
...
function AdminPanel({ settings, setSettings, exit, notify }: { settings: typeof initialSettings; setSettings: (s: typeof initialSettings) => void; exit: () => void; notify: (s: string) => void }) {
  ...
  return <div className="admin">
    <aside className="admin-sidebar"><div className="admin-brand"><Brand compact /><span>YÖNETİM</span></div><nav>{nav.map(item => <button className={section === item ? "active" : ""} onClick={() => setSection(item)} key={item}><i>{item === "Genel Bakış" ? "⌁" : item === "Sanal POS" ? "▣" : "◇"}</i>{item}<b>›</b></button>)}</nav><div className="admin-user"><span>LA</span><div><b>Lumrea Admin</b><small>Süper yönetici</small></div><button onClick={exit}>↗</button></div></aside>
    <main className="admin-main"><header><div><span className="eyebrow">LUMREA CONTROL ROOM</span><h1>{section}</h1></div><div><button className="preview-btn" onClick={exit}>Mağazayı görüntüle ↗</button><button className="admin-bell">♢<b>3</b></button></div></header>
