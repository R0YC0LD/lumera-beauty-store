...
(() => {
  ...
  function renderGenders() {
    ...
    $("#genderGrid").innerHTML = Object.entries(GENDERS).map(([key, label], i) => {
      ...
      return `<button class="gender-card" data-gender-card="${key}" style="--tone:${tones[key][0]};--tone2:${tones[key][1]}"><span class="num">0${i + 1}</span><img class="gender-photo" src="${photos[key]}" alt="${esc(label)}" loading="lazy" onerror="this.remove()"><h3>${label}</h3><p>${copy[key]}</p><small class="count">${count} ÜRÜN</small></button>`;
    }).join("");
  }
  function renderCategoryChips() {
    ...
    host.innerHTML = `<button class="chip ${!state.activeCategory ? "active" : ""}" data-category-chip="">Tümü</button>` +
      getCategories().map(c => `<button class="chip ${c === state.activeCategory ? "active" : ""}" data-category-chip="${esc(c)}">${CATEGORY_ICONS[c] ? `<i class="chip-icon">${CATEGORY_ICONS[c]}</i>` : ""}${esc(c)}</button>`).join("");
  }
  ...
  function productArt(p) {
    return p.imageUrl
      ...
      : `<div class="product-placeholder">${hangerSvg}<small>${esc((p.brand || "LUMREA").toUpperCase())}</small></div>`;
  }
  ...
  /* ---------- sipariş ---------- */
  let checkoutData = {};
  const makeOrderNo = () => `LMR-${new Date().getFullYear()}-${Date.now().toString(36).toUpperCase()}${Math.random().toString(36).slice(2, 5).toUpperCase()}`;
  ...
  function showPage(key) {
    ...
    $("#pageContent").innerHTML = `<button class="round-close modal-close" data-close>×</button><span class="eyebrow">LUMREA</span><h2>${esc(p.title)}</h2>${body || "<p>Bu içerik hazırlanıyor.</p>"}`;
    openLayer($("#pageModal"));
  }
  ...
  function initCookieBanner() {
    ...
    const saved = localStorage.getItem("lumrea_consent");
    if (!saved) banner.hidden = false;
    const decide = level => { localStorage.setItem("lumrea_consent", JSON.stringify({ level, date: new Date().toISOString() })); banner.hidden = true; };
